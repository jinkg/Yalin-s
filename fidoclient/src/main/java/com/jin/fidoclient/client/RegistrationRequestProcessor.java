...
public class RegistrationRequestProcessor {
    ...
    public RegisterOut processRequest(RegisterIn registerIn, KeyPair keyPair, byte[] keyId) {
        ...
        setAssertions(registerOut, builder, registerIn.finalChallenge, keyId);
        return registerOut;
    }

    private void setAssertions(RegisterOut registerOut, RegAssertionBuilder builder, String fcParams, byte[] keyId) {
        try {
            registerOut.assertion = builder.getAssertions(fcParams, keyId);
